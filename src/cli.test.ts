...
describe('run', () => {
  ...
  describe('--reset', () => {
    it('prints "nothing to reset" when no config file exists', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('nothing to reset'));
      expect(configModule.deleteUserConfig).not.toHaveBeenCalled();
    });
    ...
    it('prompts for confirmation and deletes config on yes', async () => {
      ...
      expect(promptModule.confirm).toHaveBeenCalled();
      expect(configModule.deleteUserConfig).toHaveBeenCalled();
      expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Settings reset'));
    });
  ...
  describe('Anthropic provider', () => {
    ...
    it('exits with code 1 when generateCommitMessage throws AnthropicError', async () => {
      vi.mocked(configModule.readUserConfig).mockReturnValue({
        provider: 'anthropic',
        model: 'claude-sonnet-4-6',
        apiKey: 'sk-ant-test',
  ...
  describe('OpenAI provider', () => {
    ...
    it('exits with code 1 when generateCommitMessage throws OpenAIError', async () => {
      vi.mocked(configModule.readUserConfig).mockReturnValue({
        provider: 'openai',
        model: 'codex-mini-latest',
        apiKey: 'sk-openai-test',
